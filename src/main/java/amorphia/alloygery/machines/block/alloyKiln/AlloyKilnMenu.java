...
import amorphia.alloygery.machines.recipe.SimpleAlloyingRecipe;
...
public class AlloyKilnMenu extends AbstractContainerMenu
{
	...
	public AlloyKilnMenu(int syncId, Inventory playerInventory, Container alloyKilnInventory, ContainerData containerData)
	{
		...
		for (int inputRow = 0; inputRow < 2; inputRow++)
		{
			for (int inputColumn = 0; inputColumn < 2; inputColumn++)
			{
				...
				this.addSlot(new Slot(alloyKilnInventory, inputColumn + inputRow * 2, 38 + inputColumn * 18, 17 + inputRow * 18));
			}
		}

		// fuel slot
		this.addSlot(new Slot(alloyKilnInventory, 4, 47, 71){
			@Override
		...
		// result slot
		this.addSlot(new Slot(alloyKilnInventory, 5, 118, 26){
	...
	public int getLitTimeRemaining()
	{
		...
		return total == 0 || remaining == 0 ? 0 : remaining * 13 / total;
	}
	...
	public int getSmeltingProgress()
	{
		...
		return duration == 0 || progress == 0 ? 0 : progress * 24 / duration;
	}
}
