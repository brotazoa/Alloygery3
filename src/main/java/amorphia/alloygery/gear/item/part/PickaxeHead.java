...
public class PickaxeHead extends ToolHeadPartItem implements GearItemTagProvider.IItemTagGen
{
    public PickaxeHead(AlloygeryMaterial material)
    {
        this(new Properties(), material);
    }

    public PickaxeHead(Properties properties, AlloygeryMaterial material)
    {
        super(properties, material, ToolTypes.PICKAXE);
    }

    public static final TagKey<Item> PICKAXE_HEAD_ITEMS_TAG = TagKey.create(Registries.ITEM, Alloygery.asResource("pickaxe_head_items"));

    @Override
    public void addItemTags(AlloygeryItemTagProvider provider, HolderLookup.Provider lookup)
    {
        provider.tagBuilderOf(PICKAXE_HEAD_ITEMS_TAG).add(this);
    }
}
