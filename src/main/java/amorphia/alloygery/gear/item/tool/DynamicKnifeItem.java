...
public class DynamicKnifeItem extends DiggerItem implements IDynamicTool, GearModelProvider.GearItemModelGenerator, GearItemTagProvider.IItemTagGen, GearRecipeProvider.IRecipeGen
{
    public static final TagKey<Block> MINEABLE_WITH_KNIFE = TagKey.create(Registries.BLOCK, Alloygery.asResource("mineable/knife"));
    public static final TagKey<Item> DYNAMIC_KNIFE_ITEMS_TAG = TagKey.create(Registries.ITEM, Alloygery.asResource("dynamic_knife_items"));

    public DynamicKnifeItem()
    {
        this(new Properties());
    }

    public DynamicKnifeItem(Properties properties)
    {
        super(0.0f, 00f, AlloygeryDefaultTier.INSTANCE, MINEABLE_WITH_KNIFE, properties);
    }

    @Override
    public void addItemTags(AlloygeryItemTagProvider provider, HolderLookup.Provider lookup)
    {
        provider.tagBuilderOf(DYNAMIC_KNIFE_ITEMS_TAG).add(this);
    ...
    {
        properties.add(AttributeProperty.of(Attributes.ATTACK_DAMAGE, PartTypes.TOOL_TYPE, PropertyOperation.BASE, 1.0f));
        properties.add(AttributeProperty.of(Attributes.ATTACK_SPEED, PartTypes.TOOL_TYPE, PropertyOperation.BASE, -0.4f));
        ...
        properties.add(AttributeProperty.of(AdditionalEntityAttributes.BONUS_RARE_LOOT_ROLLS, PartTypes.TOOL_TYPE, PropertyOperation.BASE, 2.0f));
    }
    ...
    {
        return !isBroken(dynamicGearStack) && targetBlockState.is(MINEABLE_WITH_KNIFE);
    }

    @Override
    public float getDynamicMiningSpeed(BlockState targetBlockState, ItemStack dynamicGearStack)
    {
        return targetBlockState.is(Blocks.COBWEB) ? 15.0f : isEffectiveOn(targetBlockState, dynamicGearStack) ? getMiningSpeed(dynamicGearStack) : 1.0f;
    }

    private int calculatedEnchantablity = 0;

    @Override
    public void calculateAndCacheEnchantability(ItemStack dynamicGearStack)
    {
        this.calculatedEnchantablity = EnchantabilityProperty.compute(dynamicGearStack);
    }

    @Override
    public int getEnchantmentValue()
    {
        return this.calculatedEnchantablity;
    }
